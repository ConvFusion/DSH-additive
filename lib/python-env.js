...
import { resolveGlobalInstructionPath, writeInstructionFile, } from './instructions-store.js';
 ...
 */
export function venvPythonExe(envDir) {
...
export function ensureVenv() {
    ...
    if (!sys) {
        throw new PythonEnvError('no-python', `未在本机检测到可用的 Python，无法在 ${status.dir} 创建虚拟环境。` +
    ...
    if (r.status !== 0 || !existsSync(venvPythonExe(status.dir))) {
        throw new PythonEnvError('venv-failed', `创建虚拟环境失败（${sys.command} → ${status.dir}）：${(r.stderr || r.stdout || '').trim() || 'unknown'}`);
    }
 ...
 */
export function resolvePythonEnv(config) {
...
function buildAgentsBlock(info) {
    const lines = [
        AGENTS_BLOCK_START,
        '## Python 环境',
        '',
    ...
    lines.push('  ```bash');
    for (const ex of runExample(info))
        lines.push(`  ${ex}`);
    lines.push('  ```');
    lines.push('');
 ...
 */
export async function writePythonEnvToAgents(info) {
    ...
    const snapshot = await writeInstructionFile('global', path, next, { baseSha256: null }, displayPath);
    return {

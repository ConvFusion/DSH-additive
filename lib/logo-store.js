...
function mediaByExt(ext) {
    for (const [media, e] of Object.entries(EXT_BY_MEDIA))
        if (e === ext)
            return media;
    return null;
}
export async function createLogoStore() {
    ...
    async function findCurrentFile() {
        ...
        if (!meta || typeof meta.mediaType !== 'string')
            return null;
    ...
    return {
        dir,
        async upload(bytes, declaredMediaType) {
            if (bytes.length === 0)
                throw new LogoStoreError('empty-body', 'upload body is empty');
            ...
            if (declaredMediaType && declaredMediaType !== sniffed) {
                throw new LogoStoreError('type-mismatch', `declared ${declaredMediaType} but content is ${sniffed}`);
            }
...
export function mediaTypeByExt(ext) {
    return mediaByExt(ext);
}
//# sourceMappingURL=logo-store.js.map
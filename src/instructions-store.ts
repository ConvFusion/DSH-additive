...
export type InstructionStoreErrorCode =
  | 'not-found'
  | 'not-a-directory'
  | 'too-large'
  | 'conflict'
 ...
 */
export function resolveGlobalInstructionPath(dshHome?: string): {
  path: string
  displayPath: string
} {
 ...
 */
export function resolveLocalInstructionPath(workspaceDir: string): string {
  const trimmed = workspaceDir.trim()
  if (!isAbsolute(trimmed)) {
    throw new InstructionStoreError(
      'not-found',
      `workspace directory must be an absolute path: "${workspaceDir}"`,
    )
  }
...
async function statKind(path: string): Promise<'file' | 'directory' | 'absent' | 'other'> {
  try {
...
export async function readInstructionFile(
  ...
  if (present !== 'file') {
    ...
    if (id !== 'global') {
      ...
      if (parent !== 'directory') {
        throw new InstructionStoreError(
          'not-found',
          `directory "${dirname(path)}" does not exist, so this workspace cannot be edited`,
        )
      }
    }
    return {
  ...
  if (info.size > MAX_INSTRUCTION_BYTES) {
    throw new InstructionStoreError(
      'too-large',
      `"${path}" is ${info.size} bytes, above the ${MAX_INSTRUCTION_BYTES}-byte editing limit`,
    )
  }
  ...
  if (bytes.byteLength > MAX_INSTRUCTION_BYTES) {
    throw new InstructionStoreError(
      'too-large',
      `"${path}" is ${bytes.byteLength} bytes, above the ${MAX_INSTRUCTION_BYTES}-byte editing limit`,
    )
  }
...
export async function writeInstructionFile(
  ...
  path: string,
  content: string,
  options: { baseSha256?: string | null; requireParent?: boolean } = {},
  displayPath: string = path,
): Promise<InstructionFileSnapshot> {
  ...
  if (byteLength > MAX_INSTRUCTION_BYTES) {
    throw new InstructionStoreError(
      'too-large',
      `content is ${byteLength} bytes, above the ${MAX_INSTRUCTION_BYTES}-byte limit`,
    )
  }
  ...
  if (parentKind !== 'directory') {
    if (options.requireParent === true) {
      throw new InstructionStoreError(
        'not-found',
        `directory "${parent}" does not exist, so ${LOCAL_INSTRUCTION_FILE} cannot be saved there`,
      )
    }
 ...
 */
export async function inspectWorkspaceDir(
  dir: string,
): Promise<{ path: string; missingDir: boolean }> {
  try {
